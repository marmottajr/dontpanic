 ...
 * Self-targeting destructive actions are blocked so an admin can't lock itself out.
 */
@Injectable()
export class AdminUsersService {
  ...
  async setRole(adminId: string, targetId: string, role: Role): Promise<AdminUser> {
    if (adminId === targetId) {
  ...
  async setLocked(targetId: string, locked: boolean): Promise<AdminUser> {
    await this.requireUser(targetId);
  ...
  async softDelete(adminId: string, targetId: string): Promise<void> {
    if (adminId === targetId) {
